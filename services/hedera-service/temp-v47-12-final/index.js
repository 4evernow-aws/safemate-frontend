 ...
 * Status: V47.12 - FOLDER LISTING FIX - Direct folder collection token check
 * Environment: DEV & PREPROD (dev-safemate-* / preprod-safemate-* tables)
 * Deployment: hedera-service-v47-12-folder-listing-fix.zip
 * 
 * Recent Updates:
 ...
 * - V47.12: FOLDER LISTING FIX - Direct folder collection token check (ALWAYS check known token)
 * 
 * API Endpoints:
...
async function queryUserFoldersFromBlockchain(userId) {
  try {
    ...
    // V47.12 FIX: Direct folder collection token check - ALWAYS check the known token
    // This ensures we find folders even if token relationships are not set up
    console.log(`🔍 V47.12 FIX: Direct folder collection token check...`);
    
    // V47.12: ALWAYS check the known folder collection token (0.0.6920175)
    // This ensures we find folders regardless of token relationship status
    const knownFolderTokenId = "0.0.6920175";
    console.log(`🔍 V47.12: Checking known folder collection token: ${knownFolderTokenId}`);
