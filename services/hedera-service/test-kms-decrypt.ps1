# Test KMS decryption for the user's private key
...
$byteArray = $encryptedKey.Split(',') | ForEach-Object { [byte]$_ }
$ciphertextBlob = [System.Convert]::ToBase64String($byteArray)

Write-Host "Testing KMS decryption..."
Write-Host "Ciphertext length: $($byteArray.Length) bytes"
Write-Host "KMS Key ID: $kmsKeyId"

# Test KMS decryption
try {
    $decryptResult = aws kms decrypt --key-id $kmsKeyId --ciphertext-blob $ciphertextBlob --output json
