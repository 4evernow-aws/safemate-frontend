 ...
 * Status: V42 - User Signs All Keys with Single Signature (Correct Approach)
...
async function createFolder(folderName, userId, parentFolderId = null) {
  try {
    ...
    // V42: User signs all keys with single signature (correct approach)
    console.log(`🔧 V42: Implementing user signs all keys with single signature (correct approach)...`);
    
    // Get user's public key for token creation (user will control ALL keys)
    ...
    // V42: User controls ALL keys - no operator keys needed
    console.log(`🔧 V42: User will control all keys (admin, supply, metadata, freeze, wipe, pause)`);
    ...
    const transaction = new TokenCreateTransaction()
      ...
      .setFreezeKey(userPublicKey) // User can freeze/unfreeze folder
      .setWipeKey(userPublicKey) // User can wipe folder contents if needed
      .setPauseKey(userPublicKey) // User can pause/unpause folder operations
    ...
    console.log(`🔧 V42: Signing transaction with single signature (user controls all keys)...`);
    
    // V42 FIX: Use single signature approach (correct according to Hedera docs)
    // When user controls all keys (treasury + admin), only ONE signature is needed
    // The user signs once and it satisfies both treasury and admin requirements
