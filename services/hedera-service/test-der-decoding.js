...
// The decryptedKeyBase64 is actually a base64-encoded DER structure
// We need to decode it to get the actual DER bytes
const derBytes = Buffer.from(decryptedKeyBase64, 'base64');
console.log(`DER bytes length: ${derBytes.length}`);
console.log(`DER bytes (first 20): ${Array.from(derBytes.slice(0, 20)).join(',')}`);
console.log(`DER bytes (hex): ${derBytes.toString('hex')}`);

// Now let's try to extract the private key from the DER structure
const keyBytes = derBytes;
