 ...
 * Status: V47.11 - SUBFOLDER SUPPORT - Hierarchical folder structure with parent-child relationships
 * Environment: DEV & PREPROD (dev-safemate-* / preprod-safemate-* tables)
 * Deployment: hedera-service-v47-11-subfolder-support.zip
 * 
 * Recent Updates:
 ...
 * - V47.9: FOLDER LISTING FIX - Fixed treasury account folder detection for proper folder listing
 * - V47.10: DYNAMIC FOLDER LISTING FIX - Dynamic treasury account detection using DynamoDB lookup
 * - V47.11: SUBFOLDER SUPPORT - Hierarchical folder structure with parent-child relationships and folder listing fix
 * 
 * API Endpoints:
...
/**
 * Get folder level for hierarchical structure
 */
async function getFolderLevel(folderId, client) {
  try {
    if (!folderId) return 0;
    
    // Extract token ID and serial from folder ID (format: tokenId_serial)
    const [tokenId, serial] = folderId.split('_');
    
    // Get NFT metadata
    const nftInfo = await new TokenNftInfoQuery()
      .setTokenId(TokenId.fromString(tokenId))
      .setStart(parseInt(serial))
      .setEnd(parseInt(serial))
      .execute(client);
    
    if (nftInfo.length > 0 && nftInfo[0].metadata) {
      const metadata = JSON.parse(nftInfo[0].metadata.toString());
      return metadata.l || 0; // Return level from metadata
    }
    
    return 0;
  } catch (error) {
    console.log(`⚠️ Error getting folder level for ${folderId}:`, error.message);
    return 0;
  }
}

/**
 * Build folder path for hierarchical structure
 */
async function buildFolderPath(parentFolderId, folderName, client) {
  try {
    if (!parentFolderId) return `/${folderName}`;
    
    // Extract token ID and serial from parent folder ID
    const [tokenId, serial] = parentFolderId.split('_');
    
    // Get parent NFT metadata
    const nftInfo = await new TokenNftInfoQuery()
      .setTokenId(TokenId.fromString(tokenId))
      .setStart(parseInt(serial))
      .setEnd(parseInt(serial))
    ...
    if (nftInfo.length > 0 && nftInfo[0].metadata) {
      const metadata = JSON.parse(nftInfo[0].metadata.toString());
      const parentPath = metadata.path || `/${metadata.n}`;
      return `${parentPath}/${folderName}`;
    }
    
    return `/${folderName}`;
  } catch (error) {
    console.log(`⚠️ Error building folder path for ${parentFolderId}:`, error.message);
    return `/${folderName}`;
  }
}
...
async function queryUserFoldersFromBlockchain(userId) {
  try {
    ...
    const folders = [];
    const checkedTokens = new Set(); // Track tokens we've already checked
    
    // Look for folder collection token (symbol = 'FOLDERS')
    for (const tokenId of accountInfo.tokenRelationships.keys()) {
      checkedTokens.add(tokenId.toString());
      try {
    ...
    // V47.11 FIX: Simplified folder listing - check known folder collection token
    // This handles cases where user is treasury but doesn't have token relationship yet
    console.log(`🔍 V47.11 FIX: Checking for folder collection tokens where user is treasury account...`);
    
    // Check if we found any folders from token relationships
    if (folders.length === 0) {
      console.log(`🔍 No folders found via token relationships, checking treasury account...`);
      
      // V47.11 SIMPLIFIED: Check the known folder collection token (0.0.6920175)
      // This is a temporary fix until we implement proper token discovery
      const knownFolderTokenId = "0.0.6920175";
      console.log(`🔍 V47.11: Checking known folder collection token: ${knownFolderTokenId}`);
      
      try {
        const tokenInfo = await new TokenInfoQuery()
          .setTokenId(TokenId.fromString(knownFolderTokenId))
          .execute(client);
        
        console.log(`🔍 Token info for ${knownFolderTokenId}:`, {
        ...
        // Check if user is the treasury account
        if (tokenInfo.treasuryAccountId.toString() === userWallet.hedera_account_id) {
          console.log(`✅ User is treasury account for folder collection token: ${knownFolderTokenId}`);
          
          // Check if there are any NFT serials minted
          const totalSupply = tokenInfo.totalSupply.toNumber();
          console.log(`🔍 Total supply: ${totalSupply}`);
          
          if (totalSupply > 0) {
            console.log(`✅ Found ${totalSupply} folder NFTs in collection`);
            ...
            for (let serial = 1; serial <= totalSupply; serial++) {
              try {
                ...
                if (nftInfo.length > 0 && nftInfo[0].metadata) {
                  ...
                  // Check if this is a folder (type = 'f' for compressed format)
                  if (metadata.t === 'f') {
                    const folderId = `${knownFolderTokenId}_${serial}`;
                    
                    // Handle compressed metadata format
                    const parentFolderId = metadata.p === "0" ? null : metadata.p;
                    const folderName = metadata.n || `Folder ${serial}`;
                    const folderLevel = metadata.l || 0;
                    const folderPath = metadata.path || `/${folderName}`;
                    ...
                    folders.push({
                      ...
                      balance: 1, // Treasury account owns all serials
                      metadata: metadata
                    });
                  }
                }
            ...
            console.log(`ℹ️ No folder NFTs minted yet in collection: ${knownFolderTokenId}`);
          }
        } else {
          console.log(`ℹ️ User is not treasury account for token: ${knownFolderTokenId}`);
        }
      } catch (tokenError) {
        console.log(`⚠️ Error checking known folder token:`, tokenError.message);
      }
    }
