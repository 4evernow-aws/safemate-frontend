...
if (-not $JwtToken) {
    ...
    Write-Host "   1. Login to SafeMate frontend: https://d2xl0r3mv20sy5.cloudfront.net" -ForegroundColor White
...
try {
    ...
    if ($createResponse.success) {
        ...
        Write-Host ""
        Write-Host "🔧 Next Step - Update Lambda Environment:" -ForegroundColor Cyan
        Write-Host "Run this command to update the Lambda environment variable:" -ForegroundColor White
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "Or run: .\update-lambda-env.ps1 -CollectionTokenId `"$($createResponse.tokenId)`"" -ForegroundColor Yellow
