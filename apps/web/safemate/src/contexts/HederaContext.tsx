...
export function HederaProvider({ children }: HederaProviderProps) {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      if (foldersResult.success && foldersResult.data) {
        // Check if data is directly an array (blockchain direct format)
        if (Array.isArray(foldersResult.data)) {
          foldersArray = foldersResult.data;
        }
        // Handle nested structure if it exists (fallback)
        else if (foldersResult.data && typeof foldersResult.data === 'object' && 'folders' in foldersResult.data) {
          const nestedData = foldersResult.data as any;
          if (Array.isArray(nestedData.folders)) {
            foldersArray = nestedData.folders;
          }
        }
      }

      if (foldersArray) {
        console.log('✅ Folders API call successful (blockchain direct), data:', foldersArray);
        // Transform blockchain folder data to frontend format
        const transformedFolders = foldersArray.map((folder: any): HederaFolder => ({
          id: folder.id || folder.tokenId,
          name: folder.name || folder.folderName,
          files: folder.files || [],
          subfolders: folder.subfolders || [],
          parentFolderId: folder.parentFolderId,
          hederaFileId: folder.tokenId || folder.hederaFileId,
        ...
        console.log('📁 No folders found on blockchain or API error:', foldersResult.error);
  ...
  const createFolder = async (name: string, parentFolderId?: string): Promise<string> => {
    ...
    try {
      ...
      // Refresh folders from blockchain to ensure consistency
      console.log('🔄 Refreshing folders after creation...');
      await loadUserData();
