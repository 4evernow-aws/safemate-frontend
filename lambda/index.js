 ...
 * Status: V48.1 - ENHANCED FOLDERS: Blockchain-first with DynamoDB caching
 * Environment: PREPROD (preprod-safemate-* tables)
 * Deployment: hedera-service-v48-1-enhanced.zip
 * 
 * Recent Updates:
 ...
 * - Blockchain-first storage with DynamoDB caching for performance
 ...
 * - V48.1: Added DynamoDB caching while maintaining blockchain reliability
 * 
 * Technical Fixes:
...
async function getUserFolderCollectionToken(userId, client) {
  try {
    ...
    // Look for folder collection token (symbol = 'fldr' or 'sfldr')
    for (const tokenId of accountInfo.tokenRelationships.keys()) {
      const tokenInfo = await new TokenInfoQuery()
        .setTokenId(tokenId)
        .execute(client);
      
      if (tokenInfo.symbol === 'fldr' || tokenInfo.symbol === 'sfldr') {
...
async function createFolder(folderName, userId, parentFolderId = null) {
  try {
    ...
    // V48.1: Cache the created folder in DynamoDB for performance
    const folderData = {
      success: true,
    ...
    // Cache the folder in DynamoDB
    try {
      await cacheFoldersInDynamoDB(userId, [folderData]);
      console.log(`✅ Folder cached in DynamoDB for user ${userId}`);
    } catch (cacheError) {
      console.log(`⚠️ Failed to cache folder in DynamoDB: ${cacheError.message}`);
    }
...
async function listUserFolders(userId) {
  try {
    console.log(`🔍 Listing folders for user: ${userId} (blockchain-first with DynamoDB cache)`);
    
    // Try DynamoDB cache first for performance
    try {
      const cachedResult = await listFoldersFromDynamoDB(userId);
      if (cachedResult.success && cachedResult.data.length > 0) {
        console.log(`✅ Found ${cachedResult.data.length} folders in DynamoDB cache for user ${userId}`);
        return cachedResult;
      }
    } catch (cacheError) {
      console.log(`⚠️ DynamoDB cache miss or error, falling back to blockchain: ${cacheError.message}`);
    }
    ...
    // Cache the result in DynamoDB for future requests
    try {
      await cacheFoldersInDynamoDB(userId, result.data);
    } catch (cacheError) {
      console.log(`⚠️ Failed to cache folders in DynamoDB: ${cacheError.message}`);
    }
...
// V48.1: DynamoDB caching functions for enhanced performance
async function listFoldersFromDynamoDB(userId) {
  try {
    const params = {
...
async function cacheFoldersInDynamoDB(userId, folders) {
  try {
    ...
    const existingItems = await dynamodb.scan(deleteParams).promise();
    ...
    console.log(`✅ Cached ${folders.length} folders in DynamoDB for user ${userId}`);
  } catch (error) {
    console.error('Error caching folders in DynamoDB:', error);
    throw error;
  }
}
...
function getUserFromEvent(event) {
  try {
    // Method 1: Try to get from authorizer claims (API Gateway authorizer)
    if (event.requestContext?.authorizer?.claims?.sub) {
      console.log('✅ User ID from authorizer claims:', event.requestContext.authorizer.claims.sub);
      return event.requestContext.authorizer.claims.sub;
    }
    
    // Method 2: Try to get from authorizer directly
    if (event.requestContext?.authorizer?.sub) {
      console.log('✅ User ID from authorizer:', event.requestContext.authorizer.sub);
      return event.requestContext.authorizer.sub;
    }
    
    // Method 3: Extract from JWT token in Authorization header
